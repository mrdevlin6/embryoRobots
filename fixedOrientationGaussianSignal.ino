...
int max_amp = 0;
double x0 = 5;
double x1 = 85;  //sometimes 80 40
int high_dt = 470;
int low_dt = 470;  //changes per experiment
...
Gaussian g1 = Gaussian(0, 0); //was 0, 20
...
void loop() {
  ...
  if (counter > 2) {
    ...
    // if (dt > 0) {
    //   // randNumber = 0;
    //   // setSpins(headPin, 175);
    //   setSpins(headPin, centerLine + coin * int(randNumber));  //0-255 -> 0-7.4V //this runs everything
    ...
    //       // randNumber = 0;
    //       //  setSpins(headPin, 175);
    //       setSpins(headPin, centerLine - coin * int(randNumber));
    //       // Serial.println(centerLine - coin * int(randNumber));

    //     } else {
    //       flip = false;
    //     }
    //   } else {
    //     randNumber = random(0.01, 100) / 100.00;
    //     a = pow(x1, n + 1);
    //     b = pow(x0, n + 1);
    //     c = 1 / (n + 1);
    //     d = (a - b) * randNumber + b;
